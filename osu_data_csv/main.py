...
def get_dataset(year_month: str, mode: str, set: str, dl_dir: str,
                bypass_confirm: str, cleanup: str, sql_filter: str):
    ...
    pipeline(fn, dl_dir, cleanup=cleanup == 'Y', mode=mode)#, sql_filter=sql_filter)


@click.command()
@click.option('--year_month', '-y', default=datetime.now().strftime("%Y_%m"),
              prompt=f"-y: Dataset Year & Month(YYYY_MM)")
@click.option('--mode', '-d', default="catch", prompt=f"-d: Dataset Mode (osu/taiko/catch/mania)")
...
@click.option('--sql_filter', '-f', default="")
def get_dataset_click(year_month: str, mode: str, set: str, dl_dir: str,
                      bypass_confirm: str, cleanup: str, sql_filter: str):
    get_dataset(year_month, mode, set, dl_dir, bypass_confirm, cleanup, sql_filter)
